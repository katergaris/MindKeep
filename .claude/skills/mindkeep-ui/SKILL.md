---
name: mindkeep-ui
description: Skin-agnostic behavior contract for a multi-window desktop app shell — windows, taskbar, launcher, and a deliberate mobile split-view — extracted from MindKeep's production window manager. Defines HOW windows behave (state machine, focus/z-index, drag/resize, geometry persistence, minimize/restore/maximize, cascade placement) and how desktop reshapes into mobile, independent of any visual skin. Use whenever designing or building a "gestionale" / management app, an internal tool, an admin dashboard, or any app with multiple resizable/draggable panels open at once, a taskbar-like app switcher, or a Start-menu-like launcher — on desktop AND mobile. ALWAYS pair it with a separate visual skin (windows-95-web-designer, windows-3-1-web-designer, or a new modern skin designed with the design skill) — this skill never dictates colors, bevels, gradients, shadows, or fonts. Activate on "multi-window", "window manager", "desktop shell", "taskbar", "start menu", "launcher", "gestionale", "app windowing", "draggable resizable windows", "split view on mobile", "MindKeep-style UI", even if the user never says "MindKeep" or "Windows 95" by name. NOT for single-panel apps with no overlapping windows, NOT for picking colors/fonts/visual style (that's a skin skill's job), NOT for generic responsive-grid advice unrelated to a window/taskbar/launcher metaphor.
